...
function AboutCompass({ onActivate }) {
  ...
  const rightShadeRef = useAcRef(null);
  ...
  useAcEffect(() => {
    ...
    const frame = () => {
      ...
      // matching right-side vignette so the fire behind the panel/thumbnails isn't distracting
      if (rightShadeRef.current) rightShadeRef.current.style.opacity = String(dock);
  ...
  return (
    <section className="ab-compass" ref={sectionRef} style={{ height: "calc(100vh * 9.5)" }}>
      <div className="ab-compass__stage">
        ...
        <div className="ab-compass__rightshade" ref={rightShadeRef} aria-hidden="true" />
